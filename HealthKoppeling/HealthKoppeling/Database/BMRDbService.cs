...
namespace HealthKoppeling.Database
{
    public class BMRDbService: ICosmosDbService<BMRModel>
    {
        ...
        {
            _container = cosmosClient.GetContainer(databasename, "BMRsContainer");
        }
        ...
        {
            var query = _container.GetItemQueryIterator<BMRModel>(new QueryDefinition("SELECT * FROM BMRsContainer"));
